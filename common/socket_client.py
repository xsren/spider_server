...
class SocketClient():
    ...
    def auth(self):
        ...
        try:
            self.sock.sendall("%s@@@***%s"%(self.user_name,password))
            res = self.recvall(4)
            if res != 'OK!!':
                if self.logger:
    ...
    def get_task(self, dbName, collName, count=1):
        ...
        # _req = {'data':json.dumps(req)}
        # try:
        #     res = requests.post(self.url,data=_req)
